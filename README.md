...
> Detect AI-written commits that never ran the tests they claim

**Author:** zAx4hub

## Problem

Detect AI-written commits that never ran the tests they claim. Teams often rely on closed SaaS, brittle scripts, or untested prototypes for this niche.

## Solution

`ghost-commit` is an installable TypeScript/Node toolkit by **zAx4hub** with a real **routing** engine, CLI, examples, and tests.
...
```bash
npm run demo   # or: python -m ghost_commit.cli demo
```
...
1. Core routing engine tailored to the problem
